function [A_hat ,E_hat ,iter,objective] = inexact_alm_rpca(D, lambda, tol, maxIter)
...
while ~converged       
    ...
    sigmaA = svd(A_hat);
    obj_val = sum(sigmaA) + lambda*norm(E_hat,1);
    objective{end+1} = obj_val;
    disp(["Objective Value",obj_val,iter]);
